...
// Start add for BasketOfHerbs
		public override void OnExit( Mobile m )
		{
			if ( m.Backpack != null )
			{
				foreach( BasketOfHerbs basket in m.Backpack.FindItemsByType( typeof( BasketOfHerbs ) ) )
				{
					if ( basket.IsActive )
						basket.RemoveSkillMod();
				}
			}
			base.OnExit( m );
		}

// End add for BasketOfHerbs
