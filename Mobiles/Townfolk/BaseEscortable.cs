...
namespace Server.Mobiles
{
	public class BaseEscortable : BaseCreature
	{
		...
		public virtual bool SayDestinationTo( Mobile m )
		{
			...
			if ( escorter == null )
			{
				Say( "I am looking to go to {0}, will you take me?", (dest.Name == "Ocllo" && m.Map == Map.Trammel) ? "Old Haven" : dest.Name );
				return true;
			}
			else if ( escorter == m )
			{
				Say( "Lead on! Payment will be made when we arrive in {0}.", (dest.Name == "Ocllo" && m.Map == Map.Trammel) ? "Old Haven" : dest.Name );
				return true;
			}
