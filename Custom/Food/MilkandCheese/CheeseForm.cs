		...
		protected override void OnTarget( Mobile from, object o )
		{
			if (o is MilkBucket )
			{
				...
				if (m_SautFromage.Laitage >= 15)
				{
					...
					{
						from.SendMessage(0x84C, "Tohle neni spravny milk bucket." );
				...
				{
					from.SendMessage(0x84C, "Milk bucket neobsahuje dostatek mleka." );
					from.CloseGump( typeof( CheeseFormHelpGump ) );
					from.SendGump( new CheeseFormHelpGump ( from ) );
			...
			{
				from.SendMessage(0x84C, "Pouzij milk bucket s nejmene 15 litry mleka." );
				from.CloseGump( typeof( CheeseFormHelpGump ) );
				from.SendGump( new CheeseFormHelpGump ( from ) );
			}
		}
