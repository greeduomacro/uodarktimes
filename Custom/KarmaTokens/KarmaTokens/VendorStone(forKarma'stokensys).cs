	...
	{ 
		...
		{ 
			...
			else switch( version )
			{
				...
				case 1:
				{
					bool blah = reader.ReadBool(); //for the usesbox
	...
	public class VendorStoneBuyGump : Gump
	{
		...
		private VendorStone m_Stone;
			...
			for ( int i = 0; i < m_Shopper.ItemList.Count; ++i )
			{
				...
				if ( m_Stone.Currency.ToLower() == "tokens" )
					AddLabel( 40, 40 + ((i % 5) * 20), 1152, vsi.Amount+" "+vsi.Name+" "+blessbond+" "+vsi.Price+" Token Box Tokens" );
				else
		...
		{ 
			...
			{				
				...
				{					
					Item[] tbs = from.Backpack.FindItemsByType( typeof( TokenBox ) );

					TokenBox box = null;

					foreach( TokenBox tb in tbs )
					{
						if ( from == tb.Owner )
						{
							if ( box != null && tb.Token > box.Token )
								box = tb;
							else if ( box == null )
								box = tb;
						}
					...
					if ( box == null )
					{
						from.SendMessage( "You do not have a token box." );
						return;
					}
					else if ( box.Token < m_Shopper.TotalPrice() )
					{
						from.SendMessage( "Your box does not have enouh tokens in it to buy this." );
						return;
					}

					box.Token -= m_Shopper.TotalPrice();
					...
					if ( m_Stone.Currency != null && m_Stone.Currency != "" )
						from.SendMessage( "You buy the items with the "+m_Stone.Currency+" in your tokenbox." );
					else
		...
		{ 
			...
			if ( info.ButtonID == 1 )
			{
				if ( m_Stone.Currency.ToLower() == "tokens" )
				{
					...
					else if ( box.Token < m_VSI.BBPrice )
					{
						from.SendMessage( "Your box does not have enouh tokens in it to do this." );
						return;
					}

					box.Token -= m_VSI.BBPrice;
